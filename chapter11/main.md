...
このゲームには、いくつかの重要なポイントが見受けられます。まず、Xがフォールドやコールをできるようになったことで、Yの戦略の基本構造が変わることがわかります。ゲーム#1では、Yは単に自分のベストハンドをベットして価値を高めていました。しかし、ゲーム#2では、Yがそのような戦略をとると、Xは自分の弱い手をすべてフォールドするだけで、Yが儲かることはありません。Yの応答は、自分の弱いハンドをブラフにし始め、XにYのバリューベットとYのブラフの間のハンドでコールさせ、Yにブラフをさせないようにすることでした。
